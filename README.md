...
the output of the C++ reference implementation [hh-cpp](https://github.com/censync/hh-cpp), which
owns the [specification](https://github.com/censync/hh-cpp/blob/v1.0.0/docs/SPEC.md) and the
golden vectors. `testdata/` is a byte-identical copy of those vectors; `testdata/SOURCE` names
the hh-cpp release they came from. The same pictures come from
[hh-kotlin](https://github.com/censync/hh-kotlin) (JVM and Android, Maven
`io.github.censync:hh`), [hh-ts](https://github.com/censync/hh-ts) (TypeScript, npm
`@censync/hh`) and [hh-python](https://github.com/censync/hh-python) (Python, PyPI
`humanized-hash`).
...
- **No dependencies.** `go.mod` has no `require` line. SHA-256, HMAC, CRC-32 and Adler-32 come
  from the standard library; PBKDF2, the deflate stream and the PNG, BMP and JPEG encoders, whose
  output the specification fixes byte for byte, are part of the package. It does not import
  `image/png`, `image/jpeg`, `compress/...` or `math`.
...
Nothing panics: every function returns a result or one of the package's `Err...` values, which
...
file. The API is documented on [pkg.go.dev](https://pkg.go.dev/github.com/censync/go-hh), with
runnable examples.
...
recognition in lists. See [docs/INTEGRATION.md](docs/INTEGRATION.md) for `net/http`,
