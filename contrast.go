...
// lin is LIN of SPEC.md appendix A: the sRGB transfer function times 10^6.
var lin = [256]uint32{
...
// luminance is Y of SPEC.md section 9: the relative luminance times 10^10.
func luminance(c RGB) uint64 {
	return 2126*uint64(lin[c.R]) + 7152*uint64(lin[c.G]) + 722*uint64(lin[c.B])
}

// contrastX100 is CR of SPEC.md section 9: the WCAG 2 contrast ratio times 100,
// rounded down.
func contrastX100(c1, c2 RGB) int {
	hi, lo := luminance(c1), luminance(c2)
	if hi < lo {
		hi, lo = lo, hi
	}
	return int(100 * (hi + 500000000) / (lo + 500000000))
}

// over is OVER of SPEC.md section 9: c with the alpha a laid over under.
func over(c RGB, a uint8, under RGB) RGB {
	channel := func(top, bottom uint8) uint8 {
		return uint8((int(a)*int(top) + (255-int(a))*int(bottom) + 127) / 255)
	}
	return RGB{channel(c.R, under.R), channel(c.G, under.G), channel(c.B, under.B)}
}

// figuresContrastX100 is the contrast of the weakest palette colour against
// what is seen behind the figures.
func figuresContrastX100(seen RGB) int {
	weakest := contrastX100(palette[0], seen)
	for _, c := range palette[1:] {
		if v := contrastX100(c, seen); v < weakest {
			weakest = v
		}
	}
	return weakest
...
// ContrastReport holds WCAG contrast ratios times 100, rounded down: 300 means
// 3:1.
...
// MeasureContrast measures what the options give over a page of the colour
// page; for an opaque background the page does not matter. Render refuses an
// opaque background with FiguresX100 below 200; hosts should warn below 300.
// For a translucent background pass the colour of the surface underneath.
func MeasureContrast(opts RenderOptions, page RGB) ContrastReport {
	seen := over(opts.Background.Color(), opts.Background.Alpha(), page)
	frame := over(frameColor, opts.FrameAlpha.Alpha(), seen)
	return ContrastReport{
		FiguresX100: figuresContrastX100(seen),
		FrameX100:   contrastX100(frame, seen),
	}
}
